...
function insertBaker($pdo, $firstName, $lastName, $bakeshopLocation, 
	$emailAddress, $added_by, $last_updated) {

	$sql = "INSERT INTO baker (firstName, lastName, 
		bakeshopLocation, emailAddress, added_by, last_updated) VALUES(?,?,?,?,?,?)";

	$stmt = $pdo->prepare($sql);
	$executeQuery = $stmt->execute([$firstName, $lastName, 
		$bakeshopLocation, $emailAddress, $added_by, $last_updated]);
...
function updateBaker($pdo, $firstName, $lastName, 
	$bakeshopLocation, $emailAddress, $added_by, $last_updated, $bakerID) {
...
function getPastryByBaker($pdo, $bakerID) {
	...
	$sql = "SELECT 
				...
				pastries.added_by AS added_by,
				pastries.last_updated AS last_updated
			FROM pastries
...
function insertPastry($pdo, $bakerID, $pastryName, $doughType, $sweetnessLevel, $price, $added_by, $last_updated) {
    $sql = "INSERT INTO pastries (bakerID, pastryName, doughType, sweetnessLevel, price, added_by, last_updated) 
            VALUES (?,?,?,?,?,?,?)";
    $stmt = $pdo->prepare($sql);
    $executeQuery = $stmt->execute([$bakerID, $pastryName, $doughType, $sweetnessLevel, $price, $added_by, $last_updated]);
...
function updatePastry($pdo, $bakerID, $pastryName, $doughType, $sweetnessLevel, $price, $added_by, $last_updated, $pastriesID) {
    $sql = "UPDATE pastries
            SET pastryName = ?,
                doughType = ?,
                sweetnessLevel = ?,
    ...
    $stmt = $pdo->prepare($sql);
    $stmt->execute([$pastryName, $doughType, $sweetnessLevel, $price, $added_by, $last_updated, $pastriesID, $bakerID]);
